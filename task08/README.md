...
6. Для распараллеливания напишите отдельную функция `spawn_tasks()` и вызовите её в `find_solution_parallel()` вместо отправки `find_solution()` в пул потоков.
   ...
   * Вам потребуется изменить логику `spawn_tasks()`:
     теперь в канале может быть больше одного сообщения и нас интересует не первое,
     а первое вида `Some()`.
     Вам пригодятся методы `Receiver::iter()`, `Iter::find()` и `Option::is_some()`.
7. Убедитесь, что поведение программы не изменилось.
   Скорость работы в этот момент могла незначительно увеличиться.
   Убедитесь, что задачи действительно работают параллельно: выведите на экран сообщение в начале и в конце каждой задачи.
