...
6. Когда мы распараллелим код, у нас в канале может оказаться несколько сообщений.
   ...
   2. На полученном итераторе вызовите метод `Iterator::find_map()`, который позволяет
      найти в потоке первое значения вида `Some()`.
   3. Убедитесь, что поведение программы не изменилось.
6. Вместо отправки одной задачи в пул потоков переберите значения одной из клеток поля при помощи `try_extend_field()` и
   ...
   * Обратите внимание, что при нахождении ответа функция `find_solution_parallel()` завершает свою работу и закрывает канал.
     Это может вызывать паники отправке сообщений в канал, их стоит обработать.
6. Вынесите код отправки нескольких задач в отдельную функцию `spawn_tasks()`.
...
10. Измените `SPAWN_DEPTH` с 1 на 2.
