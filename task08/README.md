...
6. Для распараллеливания напишите отдельную функция `spawn_tasks()` и вызовите её в `main()` вместо отправки `find_solution()` в пул потоков.
...
* Функции `try_extend_field`, `find_solution` и их тесты не изменены
