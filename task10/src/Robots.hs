...
-- и атаковала бы его роботом, которого вы создали в предыдущем подпункте
...
-- замыкание, которое вы реализовали в третьем подпункте и предикат isAlive так же реализованный до этого 
survivors :: [Robot]
survivors = undefined
